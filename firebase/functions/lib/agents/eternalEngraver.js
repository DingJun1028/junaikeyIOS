...
exports.eventCardTrigger = exports.eternalEngraver = void 0;
 ...
 * 負責創建和管理事件卡的智能代理
 */
exports.eternalEngraver = functions.https.onCall(async (data, context) => {
    var _a, _b;
    ...
    try {
        if (action === 'archive') {
            ...
            await db.collection('networkEvents').doc(eventId).update({
                ...
                archivedBy: ((_a = context.auth) === null || _a === void 0 ? void 0 : _a.uid) || 'system'
            });
            ...
            const eventCard = {
                ...
                payload: {
                    ...payload,
                    creator: ((_b = context.auth) === null || _b === void 0 ? void 0 : _b.uid) || 'system',
 ...
 * 事件卡觸發器 - 監聽特定集合的變化
 */
exports.eventCardTrigger = functions.firestore
 ...
 * 計算事件緊急度
 */
function calculateEventUrgency(eventType) {
    const urgencyMap = {
 ...
 * 計算事件影響度
 */
function calculateEventImpact(eventType, payload) {
    // 基於事件類型和載荷計算影響度
 ...
 * 生成事件標籤
 */
function generateEventTags(eventType, payload) {
    const tags = [];
    // 基於事件類型添加標籤
    if (eventType.includes('blueprint'))
        tags.push('blueprint');
 ...
 * 觸發緊急通知
 */
async function triggerUrgentNotification(eventId, eventCard) {
    const db = admin.firestore();
    const notification = {
        eventId,
        type: 'urgent',
 ...
 * 更新事件統計
 */
async function updateEventStatistics(eventType) {
    const db = admin.firestore();
    ...
    await db.runTransaction(async (transaction) => {
        ...
        if (!doc.exists) {
            ...
            const data = doc.data();
            const totalEvents = ((data === null || data === void 0 ? void 0 : data.totalEvents) || 0) + 1;
            const eventsByType = { ...((data === null || data === void 0 ? void 0 : data.eventsByType) || {}) };
...
//# sourceMappingURL=eternalEngraver.js.map